...
## UnitTest의 문제점
1. 무조건적인 `클래스 기반의 테스트`
2. `전체 테스트` 실행이나 `테스트 함수 하나` 실행이나 `전부 Database schema 재생성이 필요`
...
### 장점
* 간단하며 쉽게 코드 작성
* `fixture`를 통한 `기능 단위`, `인수 단위` 관리의 용이
* Test 실행시마다 Database `DDL SQL이 재실행 될 필요가 없음`
  * **미리 생성된 데이터 베이스 사용 가능** 
  * 팀에서 합의된 `Master 데이터들은` 테이블에 `미리 생성을 해놓고` 테스트도 가능  
  (Master : 서비스를 위한 규칙 같은 공통 코드 혹은 설정 등)
* `pytest` 기반의 많은 오픈 소스
...
* `fixture`에 대해서는 팀 개발자들이 `어느정도 숙지`하고 있어야 함
